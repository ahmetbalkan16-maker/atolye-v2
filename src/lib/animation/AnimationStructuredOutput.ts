...
export function createAnimationMotionPlanSystemPrompt() {
  return [
    ...
    "crop is a normalized sub-rectangle of the source image, given as fractions of its width and height.",
    `crop.x and crop.y are in [${cropNumberSpecs.x.minimum}, ${cropNumberSpecs.x.maximum}]; crop.width and crop.height are in [${cropNumberSpecs.width.minimum}, ${cropNumberSpecs.width.maximum}].`,
    "The crop rectangle must lie fully inside the image in BOTH the start and end frame: crop.x + crop.width must not exceed 1, and crop.y + crop.height must not exceed 1.",
    "To pan, drift, or reframe between start and end, change crop.x and/or crop.y and, in the same frame, reduce crop.width and/or crop.height so that crop.x + crop.width <= 1 and crop.y + crop.height <= 1 still hold. Never move crop.x or crop.y toward an edge without shrinking crop.width or crop.height to keep the sum within 1.",
    `transform.scale is in [${transformNumberSpecs.scale.minimum}, ${transformNumberSpecs.scale.maximum}]; transform.translateX and transform.translateY are in [${transformNumberSpecs.translateX.minimum}, ${transformNumberSpecs.translateX.maximum}].`,
