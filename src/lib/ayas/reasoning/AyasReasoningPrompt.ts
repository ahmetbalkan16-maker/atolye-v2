...
export function buildAyasReasoningPrompt(input: AyasReasoningPromptInput): string {
  ...
  const lines: string[] = [
    ...
    "    onun çalıştığı veya bir sonuç döndürdüğü anlamına gelmez; answer alanında \"baktım\", \"okudum\",",
    '    "kontrol ettim", "inceledim" gibi TAMAMLANMIŞ bir iddia KURMA — sen bunun gerçekten çalışıp',
    "    çalışmadığını bilmiyorsun. Bunun yerine niyet ifadesi kullan (\"bakmam gerekiyor\", \"kontrol edeyim\").",
    '    Araç gerçekten çalışırsa gerçek sonuç ayrı bir adımda kullanıcıya sunulur; answer alanın yalnızca',
    "    o adım hiç çalışmazsa gösterilecek dürüst bir yedektir.",
    '  - "read-project-document" veya "inspect-source-file" adlandırıyorsan, "toolInput" alanında hangi belgeyi/',
    '    dosyayı kastettiğini belirt: {"documentId": "checkpoint"|"roadmap"|"changelog"} veya {"filePath": "src/..."}.',
    "    Belirtmezsen bu araç çalıştırılamaz. Bu yalnızca bir ÖNERİDİR — gerçek denetim ayrıca yapılır.",
    "",
    "Bağlam:",
    ...
    '  "toolInput": {             // İSTEĞE BAĞLI — yalnızca documentId/filePath gereken bir araç adlandırdıysan',
    '    "documentId"?: string,   // "checkpoint" | "roadmap" | "changelog"',
    '    "filePath"?: string      // örn. "src/lib/ayas/AyasChatStream.ts"',
    "  },",
