...
| 7a | **C.2B.10a — DONE (Sprint 197)** — `RuntimeMigrationCandidateConsumeService`: verified candidate → empty exclusive relocation target, per-file SHA-256 no-clobber materialization, crash-safe `created→…→consumed` state machine + idempotent resume, post-copy `runtimeAuthorityProjectsContentDigest` byte-exact + inventory + manifest/backup/durable binding + read-only recovery scan. SEC1 roles `relocation-target` / `quarantine` (`assertMigrationConsumeRootsDisjoint`). C1 adapter `preflightRuntimeMigrationExternalSource` (filesystem vs repository cleanliness split). CLI `npm run runtime:migration:candidate:consume`. `cutoverAuthorized` stays `false`; the service never publishes authority or edits `.env.local`. | Runtime | HIGH | C.2B.9 | target overwrite/mix P0 | authority switch, `.env.local`, publish |
| 8 | **C.2B.11 — DONE (Sprint 198)** — Old-Root Quarantine & Rollback Contract | `finalizeOldRootQuarantine`: OS read-only barrier over the retired `projects/` tree (`FILE_ATTRIBUTE_READONLY` on every regular file; no silent fallback — `QUARANTINE_NOT_ENFORCED` if it can't be applied + verified) + append-once `quarantine-enforcement/<binding>.json` + a single-use `RuntimeAuthorityRollbackToken` bound to exactly one generation / forward transition / old root / new root / `transitionSequence` + `sourceContentDigest`. `beginTokenAuthorizedRollback → validateRollback → publishRollback → quarantineFormerTarget` (`kind: "rollback"` record, same store, same `active-authority.json` CAS): reverses authority ONLY while both roots are byte-identical to the frozen source (`ROLLBACK_TARGET_DIRTY` / `ROLLBACK_OLD_ROOT_CONTENT_DRIFT` otherwise); consumes the token append-once; quarantines the former target. Enforcement re-activates a rolled-back old root only when it carries a `quarantine-lift` record AND is the current active authority. CLI `authority:{finalize-quarantine,rollback-status,begin-rollback,validate-rollback,publish-rollback,quarantine-former-target}`. `smoke-c2b11-old-root-quarantine.ts` (15 scenarios incl. replay / generation / sequence / root / token mismatch, target-dirty, content-drift, child-process crash+restart, protected-root `authority` overlap, forward-recovery separation). | Security + Runtime | HIGH | C.2B.10 | two-root/rollback P0-P1 | Git untracking, cutover |
| 9 | **C.2B.12 — DONE (Sprint 199)** — Controlled Runtime Git Untracking | `git rm -r --cached -- data/projects` removed all 220 tracked files from the index **only** (every worktree file byte-identical to its HEAD blob — checked=220 drift=0; 2388 fs files / 611005467 bytes unchanged). `.gitignore`: the ~50-line "NOT a blanket ignore" granular section is replaced by one narrow rule — `/data/projects/` — so runtime project data can't be re-added by accident; `data/brain` (README.md tracked), `data/e2e-output` and every other `data/` subtree are unaffected. `RuntimeMigrationCandidatePreflight`'s repo-git-cleanliness check is now trivially clean (documented); `preflightRuntimeMigrationExternalSource` (C1) reports `repositoryCleanliness: "not-applicable"` and keeps the filesystem-vs-backup check separate. No runtime-authority code discovers authority via git/`.git`/`ls-files` (static smoke). `scripts/smoke-c2b12-git-untracking.ts` (7 scenarios). `cutoverAuthorized` stays `false`; no real migration / publish / `.env.local` change. | Scripts + Git/Runtime | HIGH | C.2B.11 | stale tracked runtime P0 | root switch logic |
