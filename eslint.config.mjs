import { defineConfig, globalIgnores } from "eslint/config";
import nextVitals from "eslint-config-next/core-web-vitals";
import nextTs from "eslint-config-next/typescript";

const eslintConfig = defineConfig([
  ...nextVitals,
  ...nextTs,
  // Override default ignores of eslint-config-next.
  globalIgnores([
    // Default ignores of eslint-config-next:
    ".next/**",
    "out/**",
    "build/**",
    "next-env.d.ts",
    // Local graphify tool output/bundle (already git-ignored — not project source).
    ".graphify/**",
    "graphify-out/**",
    // wrangler's own build cache/bundled output for the Cloudflare Worker
    // (already git-ignored, see .gitignore — regenerated by `wrangler dev`/
    // `wrangler deploy`, not project source; without this, running either
    // locally pollutes `npm run lint` with warnings from bundled/minified
    // copies of already-linted app source).
    "cloudflare/**/.wrangler/**",
    // Per-machine wake/STT assets: onnxruntime-web WASM glue + ONNX models,
    // staged into public/ by scripts/setup-wake-assets.ts (git-ignored).
    "public/ort/**",
    "public/wake/**",
    ".venv-wake/**",
  ]),
  { files: ["scripts/smoke-production-execution-durable-attempt.ts"], rules: { "@typescript-eslint/no-explicit-any": "off" } },
  { files: ["src/types/productionExecutionDurableAttempt.ts"], rules: { "@typescript-eslint/no-empty-object-type": "off" } },
  { files: ["src/lib/production/ProductionExecutionDurableAttempt.ts"], rules: { "@typescript-eslint/no-unused-vars": "off" } },
]);

export default eslintConfig;
