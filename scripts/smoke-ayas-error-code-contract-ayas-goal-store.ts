...
import { AyasGoalStoreError } from "../src/lib/brain/autonomy/AyasGoalStore";

/**
 * AyasGoalStoreError's own declared `code` union (src/lib/brain/autonomy/AyasGoalStore.ts) had zero direct
 ...
 */
const CODES = ["AYAS_GOAL_NOT_FOUND", "AYAS_GOAL_CORRUPT", "AYAS_GOAL_INVALID_TRANSITION", "AYAS_GOAL_IO"] as const;

// AYAS M19.3: compile-time exhaustiveness — if a future code is added to
// AyasGoalStoreError's own declared union but not to CODES above, this line
// fails `tsc --noEmit` (never silently passes at runtime).
type _AyasExpectedCode = ConstructorParameters<typeof AyasGoalStoreError>[0];
...
for (const code of CODES) {
  const error = new AyasGoalStoreError(code, `test message for ${code}`);
  assert.equal(error.code, code, `constructing with code "${code}" must carry that exact code`);
  assert.equal(error.name, "AyasGoalStoreError", "error.name must equal the class name");
  assert.ok(error instanceof Error, "must be a real Error instance");
  assert.ok(error instanceof AyasGoalStoreError, "must be an instance of its own class");
}

console.log(JSON.stringify({ status: "PASS", suite: "ayas-error-code-contract-ayas-goal-store", scenarios: CODES.length * 4 }));
