 ...
 * i-stanbul-un-fethi-1453's video stage through the real, established
 * PipelineRunner.resume() API, bounded with stopAfterStage: "video" so
 * downstream stages (assembly/thumbnail/seo/youtube/export) are never
 * touched. Sibling of apply-execute-animation-istanbul-1453.ts (that
 * script is not modified by this file) and
 * apply-execute-assembly-istanbul-1453-v2.ts.
 *
...
async function main() {
  ...
  const animationJob = await PipelineJobManager.getJob(projectSlug, `${projectSlug}-animation`);
  if (!animationJob) fail("animation job not found.");
  if (animationJob!.status !== "completed") {
    fail(`animation job.status=${animationJob!.status}, expected "completed".`);
  }
  if (animationJob!.regenerationId !== expectedRegenerationId ||
    animationJob!.generationOrdinal !== expectedGenerationOrdinal) {
    fail("animation job regeneration binding mismatch.");
  }
  console.log("animation dependency: completed, correct regeneration binding -> OK");
  ...
  console.log("\n========== 3) EXECUTE: PipelineRunner.resume(projectSlug, { stopAfterStage: 'video' }) ==========");
