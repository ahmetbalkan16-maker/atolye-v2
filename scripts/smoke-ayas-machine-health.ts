...
assert.equal(action({}), "ALLOW", "assert.equal(action({}), \"ALLOW\")");
assert.equal(action({ cpuPercent: 92 }), "THROTTLE", "assert.equal(action({ cpuPercent: 92 }), \"THROTTLE\")");
assert.equal(action({ cpuPercent: 99 }), "PAUSE", "assert.equal(action({ cpuPercent: 99 }), \"PAUSE\")");
assert.equal(action({ ramUsedPercent: 93 }), "PAUSE", "assert.equal(action({ ramUsedPercent: 93 }), \"PAUSE\")");
assert.equal(action({ vramUsedPercent: 95 }), "PAUSE", "assert.equal(action({ vramUsedPercent: 95 }), \"PAUSE\")");
assert.equal(action({ cpuPercent: 90 }), "THROTTLE", "assert.equal(action({ cpuPercent: 90 }), \"THROTTLE\")");
assert.equal(action({ diskFreePercent: 2 }), "BLOCK NEW HEAVY WORK", "assert.equal(action({ diskFreePercent: 2 }), \"BLOCK NEW HEAVY WORK\")");
assert.equal(action({ diskFreePercent: 2 }, true), "STOP OWN WORKLOAD", "assert.equal(action({ diskFreePercent: 2 }, true), \"STOP OWN WORKLOAD\")");
assert.equal(evaluateAyasMachineHealth({ ...base, cpuPercent: 99 }, { stage: "video", ownedActive: false }).mayStart, false, "assert.equal(evaluateAyasMachineHealth({ ...base, cpuPercent: 99 }, { stage: \"video\", ownedActive: false }).mayStart, false)");
