...
scenario("every category has a defined related-paths entry (possibly empty, never missing)", () => {
  for (const category of AYAS_CAPABILITY_CATEGORIES) {
    ...
    assert.ok(Array.isArray(AYAS_CAPABILITY_CATEGORY_RELATED_PATHS[category]));
  }
});

scenario("isAyasCapabilityCategory correctly validates real categories and rejects unknown strings", () => {
  assert.equal(isAyasCapabilityCategory("SUBTITLES"), true);
  assert.equal(isAyasCapabilityCategory("NOT_A_REAL_CATEGORY"), false);
  assert.equal(isAyasCapabilityCategory(""), false);
});
...
scenario("category names are unique", () => {
  assert.equal(new Set(AYAS_CAPABILITY_CATEGORIES).size, AYAS_CAPABILITY_CATEGORIES.length);
});
