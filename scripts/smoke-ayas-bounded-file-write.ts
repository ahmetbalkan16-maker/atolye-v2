...
async function main() {
  ...
  await scenario("allowed path resolves under repoRoot", () => { const r = root(); assert.equal(resolveAyasBoundedPath(r, "src/fixture.ts", ["src/"]), path.resolve(r, "src/fixture.ts")); });
  ...
  await scenario("new file requires allowCreate", async () => {
    ...
    assert.equal(fs.existsSync(path.join(r, "src/new.ts")), false);
  });
  await scenario("new file with allowCreate succeeds and returns outcomes", async () => {
    ...
    assert.equal(outcomes[0]?.beforeHash, null);
