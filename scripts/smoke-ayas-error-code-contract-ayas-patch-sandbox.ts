...
import { AyasPatchSandboxError } from "../src/lib/brain/autonomy/AyasPatchSandbox";

/**
 * AyasPatchSandboxError's own declared `code` union (src/lib/brain/autonomy/AyasPatchSandbox.ts) had zero direct
 ...
 */
const CODES = ["AYAS_SANDBOX_CREATE_FAILED", "AYAS_SANDBOX_LINK_FAILED", "AYAS_SANDBOX_DESTROY_FAILED"] as const;

// AYAS M19.3: compile-time exhaustiveness — if a future code is added to
// AyasPatchSandboxError's own declared union but not to CODES above, this line
// fails `tsc --noEmit` (never silently passes at runtime).
type _AyasExpectedCode = ConstructorParameters<typeof AyasPatchSandboxError>[0];
...
for (const code of CODES) {
  const error = new AyasPatchSandboxError(code, `test message for ${code}`);
  assert.equal(error.code, code, `constructing with code "${code}" must carry that exact code`);
  assert.equal(error.name, "AyasPatchSandboxError", "error.name must equal the class name");
  assert.ok(error instanceof Error, "must be a real Error instance");
  assert.ok(error instanceof AyasPatchSandboxError, "must be an instance of its own class");
}

console.log(JSON.stringify({ status: "PASS", suite: "ayas-error-code-contract-ayas-patch-sandbox", scenarios: CODES.length * 4 }));
